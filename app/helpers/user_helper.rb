module UserHelper
  ...
  def accept_friendship(friendship)
    ...
    link_to('Accept?', user_friendship_path(user_id: friendship.id), method: :put, class: 'tag-color')
  end
  ...
  def reject_friendship(friendship)
    ...
    link_to('Reject?', user_friendship_path(user_id: friendship.id), method: :delete, class: 'tag-color')
  end
end
